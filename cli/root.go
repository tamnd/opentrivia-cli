// Package cli assembles the opentrivia command tree from the opentrivia
...
// NewApp assembles the kit application from the opentrivia domain. The
...
//
// To add a command, declare it in opentrivia/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := opentrivia.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(opentrivia.Domain{}).Register(app)
