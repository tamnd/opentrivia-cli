...
describes what users get. Pick a format with `-o`, or let opentrivia choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
opentrivia <command> -o table   # aligned columns for reading
opentrivia <command> -o jsonl   # one JSON object per line, for piping
opentrivia <command> -o json    # a single JSON array
opentrivia <command> -o csv     # spreadsheet friendly
opentrivia <command> -o tsv     # tab-separated
opentrivia <command> -o url     # just the URL column
opentrivia <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
opentrivia <command> --fields id,title,url
```
...
```bash
opentrivia <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
opentrivia <command>            # a table, because this is a terminal
opentrivia <command> | wc -l    # JSONL, because this is a pipe
```
